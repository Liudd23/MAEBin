...
def cluster(logger, args, prefix=None):
    ...
    #用于统计 seed_namelist 中不重复的名称或标识符数量
    seed_num = len(np.unique(seed_namelist))
    # 使用带权重的种子 k-means 聚类算法，以合理的时间成本从 contig 中提取出 SCG 信息。
    ...
    for max_edges in max_edges_list:
        # 该函数使用 hnswlib 创建一个高效的 HNSW 索引，用于高维数据的近似最近邻搜索。索引构建完成后可以保存为文件，便于复用。
